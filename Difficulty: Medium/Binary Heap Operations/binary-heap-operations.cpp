...
int MinHeap::extractMin() 
{
    // Your code here
    if (heap_size <= 0) {
        return -1; // Return max value if heap is empty
    }
    if (heap_size == 1) {
...
void MinHeap::deleteKey(int i)
{
    ...
    // Extract the minimum value (which now includes the key at index `i`)
    extractMin();
}
...
void MinHeap::insertKey(int k) 
{
    // Your code here
     ...
     // If the heap is full, we cannot insert more elements
        return ;
    }
