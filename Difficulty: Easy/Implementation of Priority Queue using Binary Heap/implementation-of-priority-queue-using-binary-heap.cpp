...
class Solution {
  public:
    int extractMax() {
        // your code here
        int maxi = H[0]; //basically first element is max in max heap by drivercode
        swap(H[0],H[s]); //index start from -1 that why we take s if it start from 0->s-1
        s--;
        shiftDown(0); //making max heap so we call max heapify
        return maxi;
    }
};
